...
{
    stop_and_save_print_to_ram(0.0, -default_retraction, -default_retraction);
	...
	// {
		if (lcd_encoder != 0)
		{
			...
			if (! planner_queue_full())
			{
                ...
                current_position[E1_AXIS] += lcd_encoder;
				lcd_encoder = 0;
	...
	// {
		if (lcd_encoder != 0)
		{
			...
			if (! planner_queue_full())
			{
				current_position[E1_AXIS] += lcd_encoder;
				lcd_encoder = 0;
				plan_buffer_line_curposXYZE(manual_feedrate[E1_AXIS] / 60);
				lcd_draw_update = 1;
			}
		}
		if (lcd_draw_update)
		{
			...
			// Note: the colon behind the text is necessary to greatly shorten
			// the implementation of menu_draw_float31
			menu_draw_float31(PSTR("Extruder1:"), current_position[E1_AXIS]);
		}
		menu_back_if_clicked();
	// }
	// else
...
void lcd_move_menu_axis()
{
    ...
    MENU_ITEM_SUBMENU_P(_T(MSG_EXTRUDER1), lcd_move_e1);
	MENU_END();
}
...
void lcd_resume_print()
{
    ...
    restore_print_from_ram_and_continue(default_retraction, default_retraction);
    isPrintPaused = false;
